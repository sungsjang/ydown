...
class Settings:
    ...
    def load(cls, base_dir: Path | None = None) -> "Settings":
        ...
        return cls(
            ...
            download_dir=output,
            yt_dlp=os.environ.get("YDOWN_YTDLP", str(root / "yt-dlp.exe")).strip() or str(root / "yt-dlp.exe"),
            ffmpeg=os.environ.get("YDOWN_FFMPEG", str(root / "ffmpeg.exe")).strip() or str(root / "ffmpeg.exe"),
            ffprobe=os.environ.get("YDOWN_FFPROBE", str(root / "ffprobe.exe")).strip() or str(root / "ffprobe.exe"),
            ffmpeg_location=os.environ.get("YDOWN_FFMPEG_LOCATION", str(root)).strip() or str(root),
