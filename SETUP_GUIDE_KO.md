...
## 완료 기록을 Supabase에서 선택 삭제하기

다운로드가 완료되면 웹의 **다운로드 작업** 영역에 선택 상자가 표시됩니다.

1. 삭제하려는 완료 작업의 **삭제할 기록 선택**을 누릅니다.
2. 여러 건을 한꺼번에 고르려면 **완료 전체 선택**을 누릅니다.
3. **선택 기록 삭제**를 누릅니다.
4. 확인 창에서 삭제를 승인합니다.

이 기능은 Supabase의 완료 작업 기록과 연결된 이벤트 기록만 삭제합니다. `C:\ydownauto\downloads`에 저장된 영상과 MP3 파일은 삭제하지 않습니다. 진행 중, 대기 중, 실패 또는 취소 상태의 기록은 이 기능으로 삭제할 수 없습니다.

## yt-dlp 자동 업데이트

`ydown.exe`는 실행될 때마다 다음 공식 자체 업데이트 명령을 내부에서 실행합니다.

```powershell
C:\ydownauto\yt-dlp.exe -U
```

최신 버전이면 그대로 시작하고, 새 안정판이 있으면 업데이트한 뒤 작업 대기를 시작합니다. 인터넷 연결 문제로 업데이트 확인이 실패해도 경고를 로그에 남기고 현재 설치된 버전으로 계속 실행합니다. 결과는 `C:\ydownauto\logs\agent.log`에서 확인할 수 있습니다.
...
## 기존 설치에 YouTube 검색 기능 추가하기

기존 다운로드 기능과 `.env` 설정은 그대로 사용합니다. YouTube API 키를 만들 필요가 없습니다.

1. Supabase 프로젝트의 **SQL Editor → New query**를 엽니다.
2. `C:\ydownauto\search.sql`(저장소에서는 `database/search.sql`) 전체를 복사해 붙여 넣고 **Run**을 누릅니다. 이 파일은 기존 다운로드 기록을 지우지 않습니다. 재실행도 가능합니다.
3. GitHub의 최신 소스가 Vercel에서 배포 완료될 때까지 기다립니다.
4. 다운로드 중인 작업이 끝난 뒤 기존 `ydown.exe` 창에서 **Ctrl+C**로 종료합니다.
5. 새 `ydown.exe`를 `C:\ydownauto`에 복사합니다. `.env`, `yt-dlp.exe`, `ffmpeg.exe`, `ffprobe.exe`, 다운로드 폴더는 유지하세요.
6. `C:\ydownauto\ydown.exe`를 실행합니다. 시작 시 yt-dlp 업데이트 확인이 끝나면 검색을 받을 준비가 됩니다.
7. 웹페이지를 새로고침한 뒤 **YouTube 검색** 탭에서 검색합니다.
8. 결과에서 저장 형식(영상·MP3)을 고르고, 개별 **다운로드** 또는 체크 후 **선택 영상 다운로드**를 누릅니다.

파일은 기존 `YDOWN_DOWNLOAD_DIR` 경로로 저장됩니다. 예를 들어 `Y:\YouTube`를 설정했다면 변경되지 않습니다. 이 드라이브는 프로그램을 실행한 Windows 계정에서 연결되어 있어야 합니다.

검색은 다운로드 중에도 가능합니다. 결과는 최대 20개이며 실시간·방송 예정 영상은 제외합니다. 검색 결과는 24시간 동안 사용할 수 있으며, 만료 기록은 PC 프로그램이 실행 중일 때 자동 정리합니다. 동일 검색 결과에서 같은 영상·저장 형식을 반복 등록해도 작업은 한 번만 생깁니다. 다시 받으려면 새로 검색하세요.

- **PC의 검색 프로그램이 연결되어 있지 않음**: 검색 지원 실행파일이 실행 중인지 확인하고, 시작 시 업데이트 확인이 끝난 뒤 다시 시도하세요.
- **검색 기능 준비가 필요함 / 검색 관련 503**: 위의 `search.sql`을 올바른 Supabase 프로젝트에 실행했는지 확인하세요.
- **검색 탭이 보이지 않음**: Vercel 배포 완료 후 브라우저에서 **Ctrl+F5**로 새로고침하세요.
- 검색은 분당 6회, 대기·실행 중인 요청은 동시에 3개까지 지원합니다. 잠시 기다린 뒤 재시도하세요.
